...
messageKeyword = play_on | kick_off | goal
...
actCommand_keyword = move | dash | turn | turn_neck | change_view | attentionto | say
actCommand = OneOrMore(lp + actCommand_keyword + rp)
action = time + receive + playerName + Suppress(":") + actCommand

command = initialization | action | message | empty
line_test = command
test_action = ""
print(line_test.parseString(test_action))
